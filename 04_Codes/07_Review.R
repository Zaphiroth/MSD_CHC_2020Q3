...
msd.chpa <- chpa.format %>% 
  ...
  pivot_wider(id_cols = c(Pack_ID, ATC3_Code, Molecule_Desc, Prd_desc, 
              ...
              names_from = measure, 
              values_from = value) %>% 
  filter(Pack_ID %in% unique(msd.dashboard$Pack_ID), 
         stri_sub(quarter, 1, 4) %in% c('2019', '2020')) %>% 
  mutate(quarter = if_else(stri_sub(quarter, 1, 4) == '2019', '2019', quarter), 
         MKT = 'OAD', 
...
write.xlsx(msd.chpa, '05_Internal_Review/MSD_CHPA_2018Q1_2020Q3.xlsx')

# Update
chpa.info <- msd.chpa %>% 
  distinct(Pack_ID, ATC3, Molecule_Desc, Prod_Desc, Pck_Desc, Corp_Desc)

delivery.update <- msd.dashboard %>% 
  distinct(Pack_ID, Channel, Province, City, Date, MKT, Measurement, Value) %>% 
